...
if __name__ == "__main__":
    ...
    pristine_phonons = get_mesh_phonons(symmetry_broken_system, (3, 3, 1))
    pristine_phonons = as_pristine_gamma_phonons(pristine_phonons)
    mode_idx = np.argsort(pristine_phonons.omega)[280]

    fig, ax, anim = animate_phonon_xy(pristine_phonons[mode_idx])
    ax.set_title("Phonon Mode with Broken Symmetry")
    anim.save(
        "./examples/figures/2d_occupation.mode_animation.symmetry_broken.gif",
        dpi=300,
        writer="pillow",
    )
    ...
    for mass in [10, 20, 30, 40, 50]:
        ...
        defect_phonons = get_mesh_phonons(defect, (3, 3, 1))
    ...
    ax.legend()
    ax.set_ylim(0, 1.05)
    fig.savefig("./examples/figures/2d_occupation.survival.against_mass.png", dpi=300)
    ...
    fig.savefig("./examples/figures/2d_occupation.first_order_scatter.png", dpi=300)
